...
exports.checkSkillsSectionParseable = checkSkillsSectionParseable;
exports.checkKeywordCoverage = checkKeywordCoverage;
exports.checkNoKeywordStuffing = checkNoKeywordStuffing;
exports.runKeywordRules = runKeywordRules;
...
function checkSkillsSectionParseable(parsedResume) {
    const maxPoints = 5;
...
function checkKeywordCoverage(parsedResume, profile) {
    const maxPoints = 15;
    const fullTextLower = parsedResume.fullText.toLowerCase();
    const matchedHard = profile.hardSkillKeywords.filter((keyword) => matchesKeyword(fullTextLower, keyword.toLowerCase()));
    const matchedSoft = profile.softSkillKeywords.filter((keyword) => matchesKeyword(fullTextLower, keyword.toLowerCase()));
...
function checkNoKeywordStuffing(parsedResume, profile) {
    const maxPoints = 5;
    const fullTextLower = parsedResume.fullText.toLowerCase();
    ...
    if (parsedResume.skillsList.length > 50) {
        stuffingSignals.push(`Skills list contains ${parsedResume.skillsList.length} items (unusually large)`);
    }
...
function runKeywordRules(parsedResume, profile) {
    return [
...
//# sourceMappingURL=keywords.rules.js.map